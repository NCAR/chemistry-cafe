...
public partial class ChemistryDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Reaction>()
            ...
            .WithOne(a => a.Reaction)
            .HasForeignKey<ArrheniusParameters>(a => a.ReactionId)
            .OnDelete(DeleteBehavior.Cascade);

        // Tunneling parameters share the reaction primary key (one-to-one).
        modelBuilder.Entity<Reaction>()
            .HasOne(r => r.Tunneling)
        ...
        // First Order Loss parameters share the reaction primary key (one-to-one).
        modelBuilder.Entity<Reaction>()
            .HasOne(r => r.FirstOrderLoss)
            .WithOne(f => f.Reaction)
        ...
        // User Defined parameters share the reaction primary key (one-to-one).
        modelBuilder.Entity<Reaction>()
            .HasOne(r => r.UserDefined)
            .WithOne(u => u.Reaction)
            .HasForeignKey<UserDefinedParameters>(u => u.ReactionId)
            .OnDelete(DeleteBehavior.Cascade);
