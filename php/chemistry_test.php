...
function get_reaction_by_id() {
    ...
    $result = pg_prepare($con, "get_reactions_by_id", 
            "SELECT r.id, r.label, r.cph, r.r1, r.r2, r.r3, r.r4, r.r5, r.obsolete, r.group_id, g.ordering, g.description as section, r.wrf_custom_rate_id, reaction_products(r.id) as productstring, rrcc.rate_constant_function_id, rrcc.rate_constant_call, rcf.name, rcf.fortran_computation
             FROM reactions AS r
             INNER JOIN reaction_groups AS g 
             ON g.id=r.group_id 
             LEFT JOIN reaction_rate_constant_call as rrcc
             ON rrcc.reaction_id = r.id
             LEFT JOIN rate_constant_function as rcf
             ON rcf.id = rrcc.rate_constant_function_id
             WHERE r.id = $1;");
    ...
    while ($reaction = pg_fetch_array($chemistryreactions)) {
      ...
      $row_array['fortran'] = $reaction['fortran_computation'];
