...
/* Initialize */
    ...
    load_all_molecules = function(){
        $http.get("/php/molecules.php?action=get_all_molecules").success(function(data) {
...
/* Display Filtering */
    ...
    $scope.hideIrrelevant = function(chemistry){
        ...
        } else if ($scope.purpose ==='showReferences' || $scope.purpose ==='branchSelect' || $scope.purpose ==='groupSelect' || $scope.purpose==='editReaction') {
            return( !chemistry.obsolete ); 
    ...
    molecule_in_database = function(molecule){
       var present = (isValueInArray($scope.moleculearray, "name", molecule) || (molecule == 'M'));
    ...
    validateReactants  = function() {
        ...
        for(i=0; i<rArray.length; i++){
            ...
            if(!molecule_in_database(rArray[i]) && rArray[i] != "M"){
                alert('Reactant '+rArray[i]+' is not a valid molecule.  Please modify Reactants');
                return false;
            }
        }
        rString = rArray.join(" + ");
    ...
    $scope.editReferences = function(reaction){
        ...
        $scope.molecule = reaction.molecule;
